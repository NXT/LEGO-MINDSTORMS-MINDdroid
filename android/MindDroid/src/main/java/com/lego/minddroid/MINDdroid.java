            ...
            switch (myMessage.getData().getInt("message")) {
                ...
                case BTCommunicator.MOTOR_STATE:
                    if (myBTCommunicator != null) {
                        ...
                        int position = ByteHelper.byteToInt(motorMessage[21]) +
                                (ByteHelper.byteToInt(motorMessage[22]) << 8) +
                                (ByteHelper.byteToInt(motorMessage[23]) << 16) +
                                (ByteHelper.byteToInt(motorMessage[24]) << 24);
                ...
                case BTCommunicator.FIND_FILES:
                    if (myBTCommunicator != null) {
                        ...
                        if (programList.size() <= MAX_PROGRAMS)
                            sendBTCmessage(BTCommunicator.NO_DELAY, BTCommunicator.FIND_FILES, 1, ByteHelper.byteToInt(fileMessage[3]));
                    }
                    break;
                ...
                case BTCommunicator.SAY_TEXT:
                    if (myBTCommunicator != null) {
                        String resultText = ByteHelper.handleResult(mTts, myBTCommunicator.getReturnMessage());
